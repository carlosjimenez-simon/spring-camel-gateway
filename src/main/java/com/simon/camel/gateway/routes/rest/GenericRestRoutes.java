...
public class GenericRestRoutes extends RouteBuilder {
    ...
    public void configure() throws Exception {
        ...
        // Construcción de los endpoints estáticos para S3 con las propiedades
        // inyectadas
        ...
        from(Constants.SIMON_SPRING_CAMEL_DIRECT_FROM_PROCESAR_GENERIC_REST)
                ...
                .process(exchange -> {
                    ...
                    try {
                        ...
                        if (currentBody instanceof java.util.List) {
                            ...
                            // Iteramos de forma segura cada elemento del arreglo consolidado por el
                            // Aggregator
                            for (Object item : bodyList) {
                                if (item instanceof org.apache.camel.converter.stream.InputStreamCache
                                        || item instanceof java.io.InputStream) {
                                    // Convertimos el stream binario individual a String JSON válido usando Camel
                                    String resolvedStr = exchange.getContext().getTypeConverter()
                                            .convertTo(String.class, exchange, item);
                        ...
                        } else if (currentBody instanceof org.apache.camel.converter.stream.InputStreamCache
                                || currentBody instanceof java.io.InputStream) {
                            // === CASO UNITARIO ONLINE: Si es un stream binario de red único ===
                            cleanJsonString = exchange.getContext().getTypeConverter().convertTo(String.class, exchange,
                                    currentBody);
                        ...
                        // Re-mapeamos la estructura limpia de texto JSON a objetos POJO indexables
                        // (Map/List)
                        // Con esto, Spring Boot se encarga de pintarlo impecable con ':' y formateado
                        // en Postman
                ...
                // Una vez convertido a String, volvemos a parsearlo a Objeto de Java interno
                // (Map/List)
                // para que Postman lo renderice hermoso y AuditRoutes lo pueda procesar sin
                // romper Jackson
        ...
        from("direct:sub-procesar-request-backend")
                ...
                .process(exchange -> {
                    ...
                    if (rawRequest instanceof Map) {
                        ...
                        if (datosObj instanceof Map) {
                            ...
                            // Caso B: Viene del Splitter de Multicall (el body actual es el sub-mapa
                            // directo)
                            Map<?, ?> currentBody = exchange.getIn().getBody(Map.class);
                            if (currentBody != null) {
                ...
                .doTry()
                // Armamos las cabeceras requeridas por el componente AWS S3 de Camel para
                // descargar
                .setHeader("CamelAwsS3Key", simple(
                        "runt-cache/${header.organizacion}/${header.operacion}/${exchangeProperty.CacheFileName}"))
                ...
                // Plan de contingencia: Si no está en S3, vamos online de una para no romper la
                // experiencia
                ...
                // === LA JUGADA MAESTRA: Restauramos el body original para que Postman lo pinte
                // hermoso ===
                .setBody(exchangeProperty("ResponseToCache"))

                .log("💾 Foto de caché actualizada en S3 para la llave: ${exchangeProperty.CacheKeyValue}")
                .doCatch(Exception.class)
                // Respaldo: Si algo falla con S3, también restauramos el body para no romper la
        ...
        from("direct:invocar-backend-real")
                ...
                .process(exchange -> {
                    ...
                    // 🛠️ JUGA MAESTRA: Leer la URL del yml para ver si trae parámetros base (ej:
                    // ?companyId=...)
                    String org = exchange.getIn().getHeader("organizacion", String.class);
                    String op = exchange.getIn().getHeader("operacion", String.class);
                    String rawBaseEndpoint = exchange.getContext()
                            .resolvePropertyPlaceholders("{{simon.endpoint." + org + "." + op + "}}");
                    ...
                    if (explicitQueryParams != null && !explicitQueryParams.trim().isEmpty()
                            && !"null".equalsIgnoreCase(explicitQueryParams)) {
                        if (queryFinal.length() > 0)
                            queryFinal.append("&");
                        queryFinal.append(explicitQueryParams.trim());
                    }
                    // Opción B: Mapeo automático de datos en métodos GET (si vienen dentro del
                    // objeto datos)
                ...
                .process(exchange -> {
                    ...
                    // Si el usuario envió un path variable (ej: 000000001/transactions), le ponemos
                    // el slash inicial
                ...
                // Convierte el mapa extraído en un JSON binario real que el componente HTTP de
                // Camel necesita.
                ...
                .removeHeaders("*", "Authorization", "X-API-KEY", "X-API-Version", "MetodoDestino", "organizacion",
                        "operacion", "audit-implementation", "breadcrumbId", "CacheOnline", "CacheKeyField",
                        "Fineract-Platform-TenantId", "CamelHttpQuery", "x-Gateway-APIKey")
                ...
                .process(exchange -> {
                    String baseEndpoint = exchange.getContext().resolvePropertyPlaceholders(
                            "{{simon.endpoint." + exchange.getIn().getHeader("organizacion") + "."
                                    + exchange.getIn().getHeader("operacion") + "}}");
                    ...
                    exchange.setProperty("BridgeOptions", bridgeOptions);
                })
                ...
                .toD("${properties:simon.endpoint.${header.organizacion}.${header.operacion}}${exchangeProperty.CalculatedDynamicPath}${exchangeProperty.BridgeOptions}")
                ...
                .log(org.apache.camel.LoggingLevel.WARN,
                        "⚠️ Alerta: Circuit Breaker ejecutando Fallback para: ${header.operacion}. Causa en Exchange: ${exception.message}")
                ...
                .process(exchange -> {
                    ...
                    errorResponse.put("message",
                            "El servicio no se encuentra disponible temporalmente por protección del Gateway.");
                    ...
                    if (exception != null) {
                        ...
                        errorResponse.put("technical_reason",
                                "CallNotPermittedException (Circuito abierto rechazando peticiones)");
